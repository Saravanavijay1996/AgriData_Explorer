...
SELECT `Dist Name`,
       (AVG(CASE WHEN `Year` = (SELECT MAX(`Year`) FROM agri.agri_data) 
                 THEN `WHEAT YIELD (Kg per ha)` END) -
        AVG(CASE WHEN `Year` = (SELECT MAX(`Year`)-4 FROM agri.agri_data) 
...
SELECT `State Name`,
       ( (SUM(CASE WHEN `Year` = (SELECT MAX(`Year`) FROM agri.agri_data)
                   THEN `OILSEEDS PRODUCTION (1000 tons)` END) -
          SUM(CASE WHEN `Year` = (SELECT MAX(`Year`)-4 FROM agri.agri_data)
                   THEN `OILSEEDS PRODUCTION (1000 tons)` END)
        )
        / NULLIF(SUM(CASE WHEN `Year` = (SELECT MAX(`Year`)-4 FROM agri.agri_data)
